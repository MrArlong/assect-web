<template>
  <div style="margin-top: 50px">
    <el-form ref="productAttrForm" :model="value" label-width="120px" class="form-inner-container" size="small">
      ...
      <el-form-item label="商品详情：">
        <el-tabs v-model="activeHtmlName" type="card">
          <el-tab-pane label="电脑端详情" name="pc">
            <tinymce v-model="value.detailHtml" :width="595" :height="300" />
          </el-tab-pane>
          <el-tab-pane label="移动端详情" name="mobile">
            <tinymce v-model="value.detailMobileHtml" :width="595" :height="300" />
          </el-tab-pane>
        </el-tabs>
      </el-form-item>
    ...
    handleSyncProductSkuStock() {
      this.$confirm('将同步第一个sku的库存到所有sku,是否继续', '提示', {
        ...
        if (this.value.skuStockList !== null && this.value.skuStockList.length > 0) {
          let tempSkuList = []
          tempSkuList = tempSkuList.concat(tempSkuList, this.value.skuStockList)
          const stock = this.value.skuStockList[0].stock
          const lowStock = this.value.skuStockList[0].lowStock
          for (let i = 0; i < tempSkuList.length; i++) {
